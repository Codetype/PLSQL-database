...
CREATE OR REPLACE PROCEDURE ZMIEN_LICZBE_MIEJSC
  ...
  BEGIN
    ...
    SELECT
      CASE
        WHEN EXISTS(SELECT * FROM WYCIECZKI_MIEJSCA wm
                    WHERE wm.ID_WYCIECZKI = id_szukanej_wycieczki
                    AND ( wm.LICZBA_MIEJSC - wm.LICZBA_WOLNYCH_MIEJSC < nowa_liczba_miejsc ))
        THEN 1
        ELSE 0
    END
